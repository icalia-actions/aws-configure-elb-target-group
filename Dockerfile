...
ARG APP_PATH=/icalia-actions/aws-configure-elb-target-group
...
ARG APP_PATH=/icalia-actions/aws-configure-elb-target-group
COPY --from=builder --chown=node:node ${APP_PATH} /icalia-actions/aws-configure-elb-target-group
WORKDIR /icalia-actions/aws-configure-elb-target-group
